---
title: "Baboon Teeth Scott Scores"
author: "Gregory J. Matthews"
format: pdf
editor: visual
---

# Intro

Baboon teeth

# Questions from Department Chair Krueger

Lastly, here are the questions: - Do wear scores increase with age? (Yes, duh, but we want to test it since we modified the method) - Do wear scores differ by sex? If so, how? (Is this by a specific quadrant? Is this by a side of tooth like buccal side or lingual side?) - Are there any patterns for rate of wear? That is, is there a standard wear through time (by month or year), or are some baboons wearing their teeth down faster that what we might expect?

# Exploratory Data Analysis

```{r}
#| echo: false
#| message: false
library(tidyverse)
library(corrplot)
scott <- read.csv("/Users/gregorymatthews/Dropbox/baboon_git/scottGreg.csv")
names(scott) <-
  c(
    "id",
    "sex",
    "age_cast",
    "age_death",
    "sfbrid",
    "leftright",
    "BM_cast",
    "BD_cast",
    "LM_cast",
    "LD_cast",
    "BM_ost",
    "BD_ost",
    "LM_ost",
    "LD_ost"
  )

dentin <- read.csv("/Users/gregorymatthews/Dropbox/baboon_git/dentinGreg.csv")

names(dentin) <- c("id", "sex",
                   "Years_bw_last_mold_and_death",
                   "sfbrid",
                   "lr",
                   "dentin_area_b_death","occ_area_b_death","DER_b_death","PED_b_death",                "dentin_area_b_life","occ_area_b_life","DER_b_life","PED_b_life",
                                 "dentin_area_a_death","occ_area_a_death","DER_a_death","PED_a_death",                   "dentin_area_a_life","occ_area_a_life","DER_a_life","PED_a_life")

dentin <- dentin[,!is.na(names(dentin))]
      



b_death <- dentin %>% select(id:lr,DER_b_death) %>% pivot_longer(names_to = "var", cols = DER_b_death) 

b_life <- dentin %>% select(id:lr,DER_b_life) %>% pivot_longer(names_to = "var", cols = DER_b_life) 

a_death <- dentin %>% select(id:lr,DER_a_death) %>% pivot_longer(names_to = "var", cols = DER_a_death) 

a_life <- dentin %>% select(id:lr,DER_a_life) %>% pivot_longer(names_to = "var", cols = DER_a_life) 

#Pivot Longer
dentin_long <- bind_rows(b_death, b_life, a_death, a_life)

# Update the names of the variables
dentin_long <- dentin_long %>% rename(DER = value) %>% mutate(rater = substring(var,5,5),
     vital_status = substring(var,7,nchar(var)))

# merge on age from the scott data
dentin_long <- dentin_long %>% left_join(scott %>% select(id, age_cast, age_death), by = "id") 

#Remove records in dentin that are not in the scott score data
dentin_long <- dentin_long %>% filter(id %in% scott$id)

dentin_long$DER <- as.numeric(dentin_long$DER) 
#Data are 29.568	14.068
#But we believe this is a typo and should be 29.568	114.068
dentin_long$DER[dentin_long$id == 286 & dentin_long$rater == "a" & dentin_long$vital_status == "death"] <- 29.568/114.068

#Create age variable
dentin_long <- dentin_long %>% mutate(age = ifelse(vital_status == "death", age_death, age_cast))

#Add in dentin wear of 0 at age 42.
dentin_long <-  bind_rows(dentin_long, dentin_long %>% filter(vital_status == "death") %>% mutate(age = 42, vital_status = "origin", DER = 0))

#remove id 187
dentin_long <- dentin_long %>% filter(id != 187) 
scott <- scott %>% filter(id != 187) 



```




# Summary statistics 

```{r}
library(tidyverse)
scott <- scott %>% mutate(diff = age_death - age_cast)

qqnorm(scott$BM_cast - scott$BM_ost)
qqnorm(scott$BD_cast - scott$BD_ost)
qqnorm(scott$LM_cast - scott$LM_ost)
qqnorm(scott$LD_cast - scott$LD_ost)

#by sex
scott %>% group_by(sex) %>% 
  summarize(n = n(),
            mean_age_cast = mean(age_cast),
            mean_age_death = mean(age_death),
            mean_diff = mean(diff)) %>% as.data.frame()

#overall 
scott  %>% 
  summarize(n = n(),
            mean_age_cast = mean(age_cast),
            mean_age_death = mean(age_death),
            mean_diff = mean(diff))



```



## Variable summaries

```{r}
summary(scott)
```

## Correlations

```{r}
#| echo: false
#| message: false
#| warning: false

m <- cor(scott[,c("BM_cast","BD_cast","LM_cast","LD_cast","BM_ost","BD_ost","LM_ost","LD_ost")])

corrplot(m, method = 'square', order = 'AOE', addCoef.col = 'white')
```
```{r}
#Are the relationships linear?
ggplot(aes(x = BM_cast, y = BM_ost), data = scott) + geom_point() + geom_smooth()
ggplot(aes(x = BD_cast, y = BD_ost), data = scott) + geom_point() + geom_smooth()
ggplot(aes(x = LM_cast, y = LM_ost), data = scott) + geom_point() + geom_smooth()
ggplot(aes(x = LD_cast, y = LD_ost), data = scott) + geom_point() + geom_smooth()
```
Spearman correlations

```{r}
#| echo: false
#| message: false
#| warning: false
m <- cor(scott[,c("BM_cast","BD_cast","LM_cast","LD_cast","BM_ost","BD_ost","LM_ost","LD_ost")], method = "spearman")

corrplot(m, order = 'AOE', addCoef.col = 'black')
```

P-values for the Spearman correlation test 
```{r}
#| echo: false
#| message: false
#| warning: false
#Pair wise test
x <- scott[,c("BM_cast","BD_cast","LM_cast","LD_cast","BM_ost","BD_ost","LM_ost","LD_ost")]

pvals <- matrix(NA, nrow = 8, ncol = 8)
for (i in 1:7){
  for (j in (i+1):8){
    pvals[i,j] <- cor.test(x[,i],x[,j], method = "spearman")$p.value
  }
}

colnames(pvals) <- rownames(pvals) <- c("BM_cast","BD_cast","LM_cast","LD_cast","BM_ost","BD_ost","LM_ost","LD_ost")
pvals

#Bonferroni cut-off
```
The Bonferroni corrected cut off here is `r 0.05/choose(8,2)`.  All of these p-values are well below this cut-off.  

# Data Viz: Age vs wear by sex

```{r}
#| echo: false
#| message: false
#| warning: false
scott_long <- scott %>% pivot_longer(cols = BM_cast:LD_ost) %>% mutate(
  BL = substring(name, 1, 1),
  MD = substring(name, 2, 2),
  BLMD = substring(name, 1, 2),
  castost = substring(name, 4, nchar(name)),
  age = ifelse(castost == "cast",age_cast,age_death)
)

start <- scott_long %>% filter(castost == "ost") %>% mutate(value = 1, age = 42) %>% mutate(castost = "eruption")

scott_long <- rbind(start,scott_long)

#summary(scott)
ggplot() + geom_path(alpha = 0.5,aes(
  x = age,
  y = value,
  group = id,
  color = sex
), data = scott_long) + geom_point(alpha = 0.5,aes(
  x = age,
  y = value,
  group = id,
  color = sex
), data = scott_long) + geom_smooth(aes(
  x = age,
  y = value,
  color = sex
), data = scott_long, color = "black") + facet_grid(BL~MD + sex)
```
# Distribution of ages requested by reviewer
```{r}
#It would be good to know the distribution of male and female ages at cast and death or the distribution of the time between the two events. Knowing their means only does not allow the reader to evaluate whether or not the variation in the time between casting and death was similar in males and females.
ggplot(aes(x = age, color = sex, fill = sex), data = scott_long %>% filter(castost != "eruption")) + geom_density(alpha = 0.5) + facet_wrap(sex~castost )

ggplot(aes(x = diff, color = sex, fill = sex), data = scott %>% mutate(diff = age_death - age_cast)) + geom_histogram(alpha = 0.5) + xlab("Months from ost and cast") + facet_grid(sex~1)
  
```

## Average and standard deviation of scott scores by quadrant 

```{r}
library(tidyverse)
scott_long %>% filter(castost != "eruption") %>% group_by(sex, castost, BLMD) %>% summarize(mean = mean(value), sd = sd(value))

library(tidyverse)
scott_long %>% filter(castost != "eruption") %>% group_by(castost, BLMD) %>% summarize(mean = mean(value), sd = sd(value))

```

## Age distributions by measurement time.

```{r}
ggplot(aes(x = castost, y = age), data = scott_long %>% filter(age > 1)) + geom_boxplot() + scale_color_discrete(guide="none")
```

## Do wear scores increase with age? (Yes, duh, but we want to test it since we modified the method)

The tests below test the following hypothesis: $H_0: \mu_{cast} = \mu_{ost}$ vs $H_0: \mu_{cast} < \mu_{ost}$ for each location of the tooth individually. The test used here is a two sample dependent t-test. In all cases, the null hypothesis is rejected with a p-value less than $2.2 \times 10^{-16}$. There is statistical evidence that the mean at ost is larger than the mean at cast.

```{r}
#| message: false
#| warning: false
t.test(scott$BM_cast, scott$BM_ost, paired = TRUE, alternative = "less")
t.test(scott$BD_cast, scott$BD_ost, paired = TRUE, alternative = "less")
t.test(scott$LM_cast, scott$LM_ost, paired = TRUE, alternative = "less")
t.test(scott$LD_cast, scott$LD_ost, paired = TRUE, alternative = "less")
```

No matter what correction you use here, these are going to be highly significant.  

## Modeling effect of age

Here are pearson correlation values for you. But the relationship clearly isn't linear so these probably aren't what you want. I've also computed Spearman correlation for you, which tests for any kind of monotonic relationship. This is much closer to what you are looking for than Pearson's coefficient.

### Pearson correlation tests
I would argue there are 8 tests here in this family so the Bonferroni corrected cut off would be `r 0.05/8`.  

```{r}
#| message: false
#| warning: false
#Pearson correlation
cor.test(scott$BM_cast,scott$age_cast)
cor.test(scott$BD_cast,scott$age_cast)
cor.test(scott$LM_cast,scott$age_cast)
cor.test(scott$LD_cast,scott$age_cast)

cor.test(scott$BM_ost,scott$age_death)
cor.test(scott$BD_ost,scott$age_death)
cor.test(scott$LM_ost,scott$age_death)
cor.test(scott$LD_ost,scott$age_death)
```

### Spearman correlation tests


```{r}
#| message: false
#| warning: false
#Spearman correlation
cor.test(scott$BM_cast,scott$age_cast, method = "spearman")
cor.test(scott$BD_cast,scott$age_cast, method = "spearman")
cor.test(scott$LM_cast,scott$age_cast, method = "spearman")
cor.test(scott$LD_cast,scott$age_cast, method = "spearman")

#Corelation 
cor.test(scott$BM_ost,scott$age_death, method = "spearman")
cor.test(scott$BD_ost,scott$age_death, method = "spearman")
cor.test(scott$LM_ost,scott$age_death, method = "spearman")
cor.test(scott$LD_ost,scott$age_death, method = "spearman")



```

```{r echo = FALSE}
#| message: false
#| warning: false
out <- data.frame(castost = c(rep("cast",4), rep("ost",4)),
           BDLM = rep(c("BM","BD","LM","LD"),2), 
           pvalue = rep(NA,8))
out$pvalue[1] <- cor.test(scott$BM_cast,scott$age_cast, method = "spearman")$p.value
out$pvalue[2] <- cor.test(scott$BD_cast,scott$age_cast, method = "spearman")$p.value
out$pvalue[3] <- cor.test(scott$LM_cast,scott$age_cast, method = "spearman")$p.value
out$pvalue[4] <- cor.test(scott$LD_cast,scott$age_cast, method = "spearman")$p.value

out$pvalue[5] <- cor.test(scott$BM_ost,scott$age_death, method = "spearman")$p.value
out$pvalue[6] <- cor.test(scott$BD_ost,scott$age_death, method = "spearman")$p.value
out$pvalue[7] <- cor.test(scott$LM_ost,scott$age_death, method = "spearman")$p.value
out$pvalue[8] <- cor.test(scott$LD_ost,scott$age_death, method = "spearman")$p.value
```
### P-values for relationship between wear and age.  
I would argue there are 8 tests here in this family so the Bonferroni corrected cut off would be `r 0.05/8`.  Using the cut-off, the tests that are significant are for BM cast and age, BD cast and age, and BD ost and age.  

```{r}
out %>% pivot_wider(names_from = "BDLM", values_from = "pvalue")
```

# Do wear scores differ by sex? If so, how? (Is this by a specific quadrant? Is this by a side of tooth like buccal side or lingual side?)

If we ignore age entirely, there are significant differences in the mean vector of wear for males vs females at bost cast and ost. If we then do individual tests for wear differences by region at cast and ost separately, there are signficant differences in LM and LD at cast and LM at ost. (I used a 0.05 level of significant and a Bonferroni correction with 8 tests to gets a cut off for significance of 0.00625. This is, as always, conservative.)

```{r}
#Look for differences in male vs female with a hotelling t test
library(Hotelling)

results_cast <- hotelling.test(.~sex, data = scott %>% select(sex,BM_cast, BD_cast, LM_cast, LD_cast), var.equal = FALSE)
results_cast

results_ost <- hotelling.test(.~sex, data = scott %>% select(sex,BM_ost, BD_ost, LM_ost, LD_ost), var.equal = FALSE)
results_ost

#Now do individual t-tests on each component
t.test(BM_cast~sex,data = scott)
t.test(BD_cast~sex,data = scott)
t.test(LD_cast~sex,data = scott)
t.test(LM_cast~sex,data = scott)

#Now do individual t-tests on each component for ost 
t.test(BM_ost~sex,data = scott)
t.test(BD_ost~sex,data = scott)
t.test(LD_ost~sex,data = scott)
t.test(LM_ost~sex,data = scott)
```

<!-- # Modeling without adding in age 42. -->

<!-- But we should really be controlling for sex and age and id all at once. So we do that here using the observed data. I'm assuming linearity of the relationship between age and wear here. -->

<!-- ## BM model -->

<!-- ```{r} -->
<!-- library(lme4) -->
<!-- mod_bm <- lmer(value ~ age * sex + (1|id), data = scott_long %>% filter(BLMD == "BM" & age > 42), REML = FALSE) -->

<!-- mod_bm_red <- lmer(value ~ age + (1|id), data = scott_long %>% filter(BLMD == "BM" & age > 42), REML = FALSE) -->

<!-- #test stat -->
<!-- -2*(logLik(mod_bm_red) - logLik(mod_bm)) -->
<!-- #p-value -->
<!-- 1 - pchisq(-2*(logLik(mod_bm_red) - logLik(mod_bm)),2) -->

<!-- summary(mod_bm) -->
<!-- plot(mod_bm) -->
<!-- confint(mod_bm) -->

<!-- newx <- data.frame(age = 85:400, sex = "female") -->
<!-- newx$yhat <- (predict(mod_bm, newdata = newx, re.form = ~0)) -->

<!-- newxm <- data.frame(age = 85:400, sex = "male") -->
<!-- newxm$yhat <- (predict(mod_bm, newdata = newxm, re.form = ~0)) -->

<!-- ggplot(aes(x = age, y = yhat), data = newx) + geom_path(col = "pink", lwd = 2) + geom_point(aes(x = age, y = value, col = sex), data = scott_long %>% filter(BLMD == "BM" & age > 42)) + geom_path(aes(x = age, y = yhat), data = newxm, col = "blue", lwd = 2) -->
<!-- ``` -->

<!-- ## BD Model -->

<!-- ```{r} -->
<!-- library(lme4) -->
<!-- mod_bd <- lmer(value ~ age*sex + (1|id), data = scott_long %>% filter(BLMD == "BD"& age > 42), REML = FALSE) -->

<!-- mod_bd_red <- lmer(value ~ age + (1|id), data = scott_long %>% filter(BLMD == "BD"& age > 42), REML = FALSE) -->

<!-- #Test stat -->
<!-- -2*(logLik(mod_bd_red) - logLik(mod_bd)) -->
<!-- #p-value -->
<!-- 1 - pchisq(-2*(logLik(mod_bd_red) - logLik(mod_bd)),2) -->

<!-- summary(mod_bd) -->
<!-- plot(mod_bd) -->
<!-- confint(mod_bd) -->

<!-- newx <- data.frame(age = 1:400, sex = "female") -->
<!-- newx$yhat <- (predict(mod_bd, newdata = newx, re.form = ~0)) -->

<!-- newxm <- data.frame(age = 1:300, sex = "male") -->
<!-- newxm$yhat <- (predict(mod_bd, newdata = newxm, re.form = ~0)) -->

<!-- ggplot(aes(x = age, y = yhat), data = newx) + geom_path(col = "pink", lwd = 2) + geom_point(aes(x = age, y = value, col = sex), data = scott_long %>% filter(BLMD == "BD" & age > 1)) + geom_path(aes(x = age, y = yhat), data = newxm, col = "blue", lwd = 2) -->


<!-- ``` -->

<!-- ## LM model -->

<!-- ```{r} -->
<!-- library(lme4) -->
<!-- mod_lm <- lmer(value ~ age * sex + (1|id), data = scott_long %>% filter(BLMD == "LM" & age > 42), REML = FALSE) -->

<!-- mod_lm_red <- lmer(value ~ age + (1|id), data = scott_long %>% filter(BLMD == "LM" & age > 42), REML = FALSE) -->

<!-- #test stat -->
<!-- -2*(logLik(mod_lm_red) - logLik(mod_lm)) -->
<!-- #p-value -->
<!-- 1 - pchisq(-2*(logLik(mod_lm_red) - logLik(mod_lm)),2) -->

<!-- summary(mod_lm) -->
<!-- plot(mod_lm) -->
<!-- confint(mod_lm) -->

<!-- newx <- data.frame(age = 85:400, sex = "female") -->
<!-- newx$yhat <- (predict(mod_lm, newdata = newx, re.form = ~0)) -->

<!-- newxm <- data.frame(age = 85:400, sex = "male") -->
<!-- newxm$yhat <- (predict(mod_lm, newdata = newxm, re.form = ~0)) -->

<!-- ggplot(aes(x = age, y = yhat), data = newx) + geom_path(col = "pink", lwd = 2) + geom_point(aes(x = age, y = value, col = sex), data = scott_long %>% filter(BLMD == "LM" & age > 1)) + geom_path(aes(x = age, y = yhat), data = newxm, col = "blue", lwd = 2) -->
<!-- ``` -->

<!-- ## LD Model -->

<!-- ```{r} -->
<!-- library(lme4) -->
<!-- mod_ld <- lmer(value ~ age*sex + (1|id), data = scott_long %>% filter(BLMD == "LD"& age > 42), REML = FALSE) -->

<!-- mod_ld_red <- lmer(value ~ age + (1|id), data = scott_long %>% filter(BLMD == "LD"& age > 42), REML = FALSE) -->

<!-- #Test stat -->
<!-- -2*(logLik(mod_ld_red) - logLik(mod_ld)) -->
<!-- #p-value -->
<!-- 1 - pchisq(-2*(logLik(mod_ld_red) - logLik(mod_ld)),2) -->

<!-- summary(mod_ld) -->
<!-- plot(mod_ld) -->
<!-- confint(mod_lm) -->

<!-- newx <- data.frame(age = 85:400, sex = "female") -->
<!-- newx$yhat <- (predict(mod_ld, newdata = newx, re.form = ~0)) -->

<!-- newxm <- data.frame(age = 85:300, sex = "male") -->
<!-- newxm$yhat <- (predict(mod_ld, newdata = newxm, re.form = ~0)) -->

<!-- ggplot(aes(x = age, y = yhat), data = newx) + geom_path(col = "pink", lwd = 2) + geom_point(aes(x = age, y = value, col = sex), data = scott_long %>% filter(BLMD == "LD" & age > 1)) + geom_path(aes(x = age, y = yhat), data = newxm, col = "blue", lwd = 2) -->


<!-- ``` -->

# Modeling

## Scott Scores

But we should really be controlling for sex and age and id all at once and I think we should be looking at age at eruption with wear of 0.

Need to uses REML = FALSE to compare model.  Reason why:   -https://stats.stackexchange.com/questions/272633/how-to-decide-whether-to-set-reml-to-true-or-false/272654#272654
 - https://stackoverflow.com/questions/54980399/when-should-i-specify-reml-false-in-lmer

## Scott Score overall 

```{r}
library(lme4)
scott_long_sum <- scott_long %>% group_by(id, castost, age, sex) %>% summarize(value = sum(value)) 

scott_long_sum %>% group_by(castost, sex) %>% summarize(mean = mean(value), std = sd(value))

scott_long_sum %>% group_by(castost) %>% summarize(mean = mean(value), std = sd(value))

mod_all_full <- lmer(value ~ (age+I(age^2) + I(age^3))*sex + (1|id), data = scott_long_sum, REML = FALSE)

mod_all_red <- lmer(value ~ (age+I(age^2)+ I(age^3)) + (1|id), data = scott_long_sum, REML = FALSE)

#ICC Ovrall
(VarCorr(mod_all_full)$id[1])/(VarCorr(mod_all_full)$id[1]+attr(VarCorr(mod_all_full),"sc")^2)


#test stat
-2*(logLik(mod_all_red) - logLik(mod_all_full))
#p-value
1 - pchisq(-2*(logLik(mod_all_red) - logLik(mod_all_full)),4)

summary(mod_all_full)
plot(mod_all_full)
confint(mod_all_full) 
qqnorm(residuals(mod_all_full))
hist(residuals(mod_all_full))
shapiro.test(residuals(mod_all_full))

newx <- data.frame(age = 42:400, sex = "female")
newx$yhat <- (predict(mod_all_full, newdata = newx, re.form = ~0))

newxm <- data.frame(age = 42:300, sex = "male")
newxm$yhat <- (predict(mod_all_full, newdata = newxm, re.form = ~0))

ggplot(aes(x = age, y = yhat), data = newx) + geom_path(col = "pink", lwd = 2) + geom_point(aes(x = age, y = value, col = sex), data = scott_long_sum) + geom_path(aes(x = age, y = yhat), data = newxm, col = "blue", lwd = 2) + ggtitle("Scott Score - Overall")
```

## BM model

```{r}
library(lme4)
mod_bm <- lmer(value ~ (age+I(age^2) + I(age^3))*sex + (1|id), data = scott_long %>% filter(BLMD == "BM"), REML = FALSE)

mod_bm_red <- lmer(value ~ (age+I(age^2)+ I(age^3)) + (1|id), data = scott_long %>% filter(BLMD == "BM"), REML = FALSE)

#ICC BM
(VarCorr(mod_bm)$id[1])/(VarCorr(mod_bm)$id[1]+attr(VarCorr(mod_bm),"sc")^2)

#test stat
-2*(logLik(mod_bm_red) - logLik(mod_bm))
#p-value
1 - pchisq(-2*(logLik(mod_bm_red) - logLik(mod_bm)),4)

summary(mod_bm)
plot(mod_bm)
confint(mod_bm) 
qqnorm(residuals(mod_bm))
shapiro.test(residuals(mod_bm))

newx <- data.frame(age = 42:400, sex = "female")
newx$yhat <- (predict(mod_bm, newdata = newx, re.form = ~0))

newxm <- data.frame(age = 42:300, sex = "male")
newxm$yhat <- (predict(mod_bm, newdata = newxm, re.form = ~0))

ggplot(aes(x = age, y = yhat), data = newx) + geom_path(col = "pink", lwd = 2) + geom_point(aes(x = age, y = value, col = sex), data = scott_long %>% filter(BLMD == "BM")) + geom_path(aes(x = age, y = yhat), data = newxm, col = "blue", lwd = 2) + ggtitle("Scott Score - BM")
```

## BD Model

```{r}
library(lme4)
mod_bd <- lmer(value ~ (age+I(age^2)+ I(age^3))*sex + (1|id), data = scott_long %>% filter(BLMD == "BD"), REML = FALSE)

mod_bd_red <- lmer(value ~ (age+I(age^2)+ I(age^3)) + (1|id), data = scott_long %>% filter(BLMD == "BD"), REML = FALSE)

#ICC BD
(VarCorr(mod_bd)$id[1])/(VarCorr(mod_bd)$id[1]+attr(VarCorr(mod_bd),"sc")^2)

#Test stat
-2*(logLik(mod_bd_red) - logLik(mod_bd))
#p-value
1 - pchisq(-2*(logLik(mod_bd_red) - logLik(mod_bd)),4)

summary(mod_bd)
plot(mod_bd)
confint(mod_bd)
qqnorm(residuals(mod_bd))
shapiro.test(residuals(mod_bd))


newx <- data.frame(age = 42:400, sex = "female")
newx$yhat <- (predict(mod_bd, newdata = newx, re.form = ~0))

newxm <- data.frame(age = 42:300, sex = "male")
newxm$yhat <- (predict(mod_bd, newdata = newxm, re.form = ~0))

ggplot(aes(x = age, y = yhat), data = newx) + geom_path(col = "pink", lwd = 2) + geom_point(aes(x = age, y = value, col = sex), data = scott_long %>% filter(BLMD == "BD")) + geom_path(aes(x = age, y = yhat), data = newxm, col = "blue", lwd = 2) + ggtitle("Scott Score - BD")


```

## LM model

```{r}
library(lme4)
mod_lm <- lmer(value ~ (age+I(age^2)+ I(age^3))*sex + (1|id), data = scott_long %>% filter(BLMD == "LM"), REML = FALSE)

mod_lm_red <- lmer(value ~ (age+I(age^2)+ I(age^3)) + (1|id), data = scott_long %>% filter(BLMD == "LM"), REML = FALSE)

#ICC LM
(VarCorr(mod_lm)$id[1])/(VarCorr(mod_lm)$id[1]+attr(VarCorr(mod_lm),"sc")^2)

#test stat
-2*(logLik(mod_lm_red) - logLik(mod_lm))
#p-value
1 - pchisq(-2*(logLik(mod_lm_red) - logLik(mod_lm)),4)

summary(mod_lm)
plot(mod_lm)
confint(mod_lm)

newx <- data.frame(age = 1:400, sex = "female")
newx$yhat <- (predict(mod_lm, newdata = newx, re.form = ~0))

newxm <- data.frame(age = 1:300, sex = "male")
newxm$yhat <- (predict(mod_lm, newdata = newxm, re.form = ~0))

ggplot(aes(x = age, y = yhat), data = newx) + geom_path(col = "pink") + geom_point(aes(x = age, y = value, col = sex), data = scott_long %>% filter(BLMD == "LM")) + geom_path(aes(x = age, y = yhat), data = newxm, col = "blue") + ggtitle("Scott Score - LM")
```

## LD Model

```{r}
library(lme4)
mod_ld <- lmer(value ~ (age+I(age^2)+ I(age^3))*sex + (1|id), data = scott_long %>% filter(BLMD == "LD"), REML = FALSE)

mod_ld_red <- lmer(value ~ (age+I(age^2)+ I(age^3)) + (1|id), data = scott_long %>% filter(BLMD == "LD"), REML = FALSE)

#ICC LD
(VarCorr(mod_ld)$id[1])/(VarCorr(mod_ld)$id[1]+attr(VarCorr(mod_ld),"sc")^2)

#Test stat
-2*(logLik(mod_ld_red) - logLik(mod_ld))
#p-value
1 - pchisq(-2*(logLik(mod_ld_red) - logLik(mod_ld)),4)

summary(mod_ld)
plot(mod_ld)
confint(mod_bm)

newx <- data.frame(age = 42:400, sex = "female")
newx$yhat <- (predict(mod_ld, newdata = newx, re.form = ~0))

newxm <- data.frame(age = 42:300, sex = "male")
newxm$yhat <- (predict(mod_ld, newdata = newxm, re.form = ~0))

ggplot(aes(x = age, y = yhat), data = newx) + geom_path(col = "pink", lwd = 2) + geom_point(aes(x = age, y = value, col = sex), data = scott_long %>% filter(BLMD == "LD")) + geom_path(aes(x = age, y = yhat), data = newxm, col = "blue", lwd = 2) + ggtitle("Scott Score - LD")


```

## Dentin Ratio 
```{r}
ggplot(aes(x = age, y = DER, color = vital_status), data = dentin_long) + geom_point() + geom_smooth()

library(lme4)
mod_dentin_full <- lmer(DER ~ (age+I(age^2)+ I(age^3))*sex + (1|id) + (1|rater), data = dentin_long, REML = FALSE)

mod_dentin_red <- lmer(DER ~ (age+I(age^2)+ I(age^3)) + (1|id) + (1|rater), data = dentin_long, REML = FALSE)



#Summary stats for dentin
#By sex
dentin_long %>% group_by(id, vital_status, sex) %>% summarize(DER = mean(DER)) %>% ungroup() %>% group_by(vital_status, sex) %>% summarize(mean = mean(DER), std = sd(DER))

#Overall 
dentin_long %>% group_by(id, vital_status, sex) %>% summarize(DER = mean(DER)) %>% ungroup() %>% group_by(vital_status) %>% summarize(mean = mean(DER), std = sd(DER))

#comparing DER by sex
library(lme4)
library(lmerTest)
mod_sex_life <- lmer(DER ~  sex + (1|rater) + (1|id), data = dentin_long %>% filter(vital_status == "life"))
summary(mod_sex_life)

mod_sex_death <- lmer(DER ~  sex + (1|rater) + (1|id), data = dentin_long %>% filter(vital_status == "death"))
summary(mod_sex_death)


#comparing DER by vital status
library(lme4)
mod1 <- lmer(DER ~  vital_status + (1|rater) + (1|id), data = dentin_long %>% filter(vital_status != "origin"))
summary(mod1)

#comparing DER by vital status using diff
death <- dentin_long %>% filter(vital_status == "death") %>% select(id, rater, DER, vital_status)
life <- dentin_long %>% filter(vital_status == "life") %>% select(id, rater, DER, vital_status)

merg <- death %>% left_join(life, by = c("id","rater")) %>% mutate(diff = DER.x - DER.y)

library(lmerTest)
mod1 <- lmer(diff ~ 0 + (1|rater), data = merg)
mod2 <- lmer(diff ~ 1 + (1|rater), data = merg)
#Is the intercept significantly different than 0?
anova(mod2, mod1)







newx <- data.frame(age = 42:400, sex = "female")
newx$yhat <- (predict(mod_dentin_full, newdata = newx, re.form = ~0))

newxm <- data.frame(age = 42:300, sex = "male")
newxm$yhat <- (predict(mod_dentin_full, newdata = newxm, re.form = ~0))

ggplot(aes(x = age, y = yhat), data = newx) +
  geom_point(aes(x = age, y = DER, col = sex), data = dentin_long) + geom_path(col = "pink", lwd = 2)  + geom_path(aes(x = age, y = yhat), data = newxm, col = "blue", lwd = 2)


##ICC analysis 
## Only use death and cast ages
mod_dentin_full <- lmer(DER ~ (age+I(age^2)+ I(age^3))*sex + (1|id) + (1|rater), data = dentin_long, REML = FALSE)

#mod_dentin_red <- lmer(DER ~ (age+I(age^2)+ I(age^3)) + (1|id) + (1|rater), data = dentin_long %>% filter(vital_status %in% c("death","life")), REML = FALSE)


(VarCorr(mod_dentin_full)$rater[1])/(VarCorr(mod_dentin_full)$rater[1] + VarCorr(mod_dentin_full)$id[1] +attr(VarCorr(mod_dentin_full),"sc")^2)

(VarCorr(mod_dentin_full)$id[1])/(VarCorr(mod_dentin_full)$rater[1] + VarCorr(mod_dentin_full)$id[1] +attr(VarCorr(mod_dentin_full),"sc")^2)

```


# Are there any patterns for rate of wear? That is, is there a standard wear through time (by month or year), or are some baboons wearing their teeth down faster that what we might expect?

# Pava

## Dentin ratio

```{r}
#Overall 
library(Iso)
#Separate curves for male and female.  
sub <- dentin_long %>% filter(sex == "male") %>% arrange(age) 
y <- dentin_long %>% filter( sex == "male") %>% arrange(age) %>% pull(DER)
sub$isoy <- pava(y)
submale <- sub

sub <- dentin_long %>% filter( sex == "female") %>% arrange(age)
y <- dentin_long %>% filter(sex == "female") %>% arrange(age) %>% pull(DER)
sub$isoy <- pava(y)
subfemale <- sub

sub_dentin <- rbind(submale, subfemale)

ggplot(aes(x = age, y = DER, color = sex), data = sub_dentin) + geom_point(alpha = 0.5) + geom_path(aes(x = age, y = isoy), data = sub_dentin, lwd = 2) + ggtitle("Dentin Ratio") + theme_bw()

pdf(file = "/Users/gregorymatthews/Dropbox/baboon_git/isotonic_dentin_qword.pdf", h = 5, w = 5, pointsize = 1/600)
ggplot() + geom_point(aes(y = DER, x = age, color = sex),data = sub_dentin,size = 0.5) + geom_path(aes(x = age, y = isoy, color = sex), data = sub_dentin, lwd = 2) + theme_bw() + ylab("Dentin Ratio") + xlab("Age (Months)") + scale_colour_manual(
  values = c("#F8766D","#00BFC4"),
  labels = c( "Female","Male")
)
dev.off()
```


## Scott scores

## Scott scores summed across locations
```{r}
#Overall 
library(Iso)

#Isotonic regression for overall
scott_long_sum <- scott_long %>% group_by(id, castost, age, sex) %>% summarize(value = sum(value)) 

#Separate curves for male and female.  
sub <- scott_long_sum %>% filter(sex == "male") %>% arrange(age) 
y <- scott_long_sum %>% filter(sex == "male") %>% arrange(age) %>% pull(value)
sub$isoy <- pava(y)
submale <- sub

sub <- scott_long_sum %>% filter(sex == "female") %>% arrange(age)
y <- scott_long_sum %>% filter(sex == "female") %>% arrange(age) %>% pull(value)
sub$isoy <- pava(y)
subfemale <- sub

suball <- rbind(submale, subfemale)

ggplot(aes(x = age, y = value, color = sex), data = suball) + geom_point(alpha = 0.5) + geom_path(aes(x = age, y = isoy), data = suball, lwd = 2) + ggtitle("overall") + ylim(0,40)

pdf(file = "/Users/gregorymatthews/Dropbox/baboon_git/isotonic_scottsum_qword.pdf", h = 5, w = 5, pointsize = 1/600)
ggplot() + geom_point(aes(y = value, x = age, color = sex),data = suball,size = 0.5) + geom_path(aes(x = age, y = isoy, color = sex), data = suball, lwd = 2) + theme_bw() + ylab("Sum of Krueger-Scott Score") + xlab("Age (Months)") + scale_colour_manual(
  values = c("#F8766D","#00BFC4"),
  labels = c( "Female","Male")
)
dev.off()
```


## Individual locations 

```{r}

library(Iso)

#Isotonic regression for BM.  
#Separate curves for male and female.  
type <- "BM"
sub <- scott_long %>% filter(BLMD == type & sex == "male") %>% arrange(age) 
y <- scott_long %>% filter(BLMD == type & sex == "male") %>% arrange(age) %>% pull(value)
sub$isoy <- pava(y)
submale <- sub

sub <- scott_long %>% filter(BLMD == type & sex == "female") %>% arrange(age)
y <- scott_long %>% filter(BLMD == type & sex == "female") %>% arrange(age) %>% pull(value)
sub$isoy <- pava(y)
subfemale <- sub

subBM <- rbind(submale, subfemale)

ggplot(aes(x = age, y = value, color = sex), data = subBM) + geom_point(alpha = 0.5) + geom_path(aes(x = age, y = isoy), data = subBM, lwd = 2) + ggtitle(type)

#Isotonic regression for BD.  
#Separate curves for male and female.  
type <- "BD"
sub <- scott_long %>% filter(BLMD == type & sex == "male") %>% arrange(age) 
y <- scott_long %>% filter(BLMD == type & sex == "male") %>% arrange(age) %>% pull(value)
sub$isoy <- pava(y)
submale <- sub

sub <- scott_long %>% filter(BLMD == type & sex == "female") %>% arrange(age)
y <- scott_long %>% filter(BLMD == type & sex == "female") %>% arrange(age) %>% pull(value)
sub$isoy <- pava(y)
subfemale <- sub

subBD <- rbind(submale, subfemale)

ggplot(aes(x = age, y = value, color = sex), data = subBD) + geom_point(alpha = 0.5) + geom_path(aes(x = age, y = isoy), data = subBD, lwd = 2) + ggtitle(type)

#Isotonic regression for LD.  
#Separate curves for male and female.  
type <- "LD"
sub <- scott_long %>% filter(BLMD == type & sex == "male") %>% arrange(age) 
y <- scott_long %>% filter(BLMD == type & sex == "male") %>% arrange(age) %>% pull(value)
sub$isoy <- pava(y)
submale <- sub

sub <- scott_long %>% filter(BLMD == type & sex == "female") %>% arrange(age)
y <- scott_long %>% filter(BLMD == type & sex == "female") %>% arrange(age) %>% pull(value)
sub$isoy <- pava(y)
subfemale <- sub

subLD <- rbind(submale, subfemale)

ggplot(aes(x = age, y = value, color = sex), data = subLD) + geom_point(alpha = 0.5) + geom_path(aes(x = age, y = isoy), data = subLD, lwd = 2) + ggtitle(type)


#Isotonic regression for LM.  
#Separate curves for male and female.  
type <- "LM"
sub <- scott_long %>% filter(BLMD == type & sex == "male") %>% arrange(age) 
y <- scott_long %>% filter(BLMD == type & sex == "male") %>% arrange(age) %>% pull(value)
sub$isoy <- pava(y)
submale <- sub

sub <- scott_long %>% filter(BLMD == type & sex == "female") %>% arrange(age)
y <- scott_long %>% filter(BLMD == type & sex == "female") %>% arrange(age) %>% pull(value)
sub$isoy <- pava(y)
subfemale <- sub

subLM <- rbind(submale, subfemale)

ggplot(aes(x = age, y = value, color = sex), data = subLM) + geom_point(alpha = 0.5) + geom_path(aes(x = age, y = isoy), data = subLM, lwd = 2) + ggtitle(type)



suball <- rbind(subBM, subBD, subLM, subLD)

# ggplot() + geom_point(aes(y = value, x = age, color = sex),data = suball,alpha = 0.5) + geom_path(aes(x = age, y = isoy, color = sex), data = suball, lwd = 2) + facet_grid(BL~MD)

suball <- suball %>% mutate(BLword = case_when(BL == "B" ~ "Buccal",
                                     BL == "L" ~ "Lingual"),
                            MDword = case_when(MD == "M" ~ "Mesial",
                                     MD == "D" ~ "Distal"))

suball$MDword <- factor(suball$MDword, levels = c("Mesial","Distal"))


pdf(file = "/Users/gregorymatthews/Dropbox/baboon_git/isotonic.pdf", h = 5, w = 5, pointsize = 1/600)
ggplot() + geom_point(aes(y = value, x = age, color = sex),data = suball, size = 0.5) + geom_path(aes(x = age, y = isoy, color = sex), data = suball, lwd = 2) + facet_grid(MD~BL) + theme_bw() + ylab("Krueger-Scott Score") + xlab("Age (Months)") + scale_colour_manual(
  values = c("#F8766D","#00BFC4"),
  labels = c( "Female","Male")
)
dev.off()

pdf(file = "/Users/gregorymatthews/Dropbox/baboon_git/isotonic_qword.pdf", h = 5, w = 5, pointsize = 1/600)
ggplot() + geom_point(aes(y = value, x = age, color = sex),data = suball,size = 0.5) + geom_path(aes(x = age, y = isoy, color = sex), data = suball, lwd = 2) + facet_grid(MDword~BLword) + theme_bw() + ylab("Krueger-Scott Score") + xlab("Age (Months)") + scale_colour_manual(
  values = c("#F8766D","#00BFC4"),
  labels = c( "Female","Male")
)
dev.off()


# 
# type <- "BD"
# sub <- scott_long %>% filter(BLMD == type) %>% arrange(age) 
# y <- scott_long %>% filter(BLMD == type) %>% arrange(age) %>% pull(value)
# sub$isoy <- pava(y)
# ggplot(aes(x = age, y = value), data = sub) + geom_point() + geom_path(aes(x = age, y = isoy), data = sub, color = "red") + ggtitle(type)
# 
# type <- "LM"
# sub <- scott_long %>% filter(BLMD == type) %>% arrange(age) 
# y <- scott_long %>% filter(BLMD == type) %>% arrange(age) %>% pull(value)
# sub$isoy <- pava(y)
# ggplot(aes(x = age, y = value), data = sub) + geom_point() + geom_path(aes(x = age, y = isoy), data = sub, color = "red") + ggtitle(type)
# 
# type <- "LD"
# sub <- scott_long %>% filter(BLMD == type) %>% arrange(age) 
# y <- scott_long %>% filter(BLMD == type) %>% arrange(age) %>% pull(value)
# sub$isoy <- pava(y)
# ggplot(aes(x = age, y = value), data = sub) + geom_point() + geom_path(aes(x = age, y = isoy), data = sub, color = "red") + ggtitle(type)

```

<!-- # GCREG -->

<!-- ```{r} -->

<!-- #https://github.com/bonStats/gcreg?tab=readme-ov-file -->

<!-- #Last link in github repo: file:///Users/gregorymatthews/Downloads/monotone-constrained-mixed-effects-models.pdf -->

<!-- library(gcreg) -->

<!-- library(fda) -->

<!-- library(tidyverse) -->

<!-- type <- "BM" -->

<!-- sub <- scott_long %>% filter(BLMD == type) %>% arrange(age)  -->

<!-- test <- cpm(value~age , data = sub, degree = 3, constraint = "monotone", c_region = c(1,404)) -->

<!-- sc <- gen_scale_data_funs(sub$age) -->

<!-- sub$age_scaled <- sc$scale(sub$age) -->

<!-- sc <- gen_scale_data_funs(sub$value) -->

<!-- sub$value_scaled <- sc$scale(sub$value) -->

<!-- md <- gcreg:::make_em_model_specs(value_scaled~age_scaled, data = sub, p_degree = 8, r_degree = 3, r_constrained = F, mcontr_region = c(-1,1), group_name = "id") -->

<!-- quang <- gcreg:::constrained_lmm_em(model = md, tol = 0.1, verbose = T, save_steps = F) -->

<!-- ``` -->

#Comparing scott and DER
```{r}
#sum the scott scorres
#Average the dentin across raters.
#merge the scottt and DER data
#Plot scott vs DER.  

scott_long_sum <- scott_long %>% group_by(id, castost, age, sex) %>% summarize(value = sum(value))

dentin_long_sum <- dentin_long %>% group_by(id, age) %>% summarize(DER = mean(DER))

merg <- scott_long_sum %>% left_join(dentin_long_sum, by = c("id", "age"))

ggplot(aes(x = value, y = DER),data = merg) + geom_point() + xlab("Krueger-Scott Score") + geom_smooth() + theme_bw()

ggplot(aes(x = value, y = DER),data = merg) + geom_point() + xlab("Krueger-Scott Score") + geom_smooth() + theme_bw() + facet_grid(~sex)
```

# Notes for Greg

-   https://link.springer.com/article/10.1007/s11222-017-9797-8
-   https://academic.oup.com/jrsssb/article/83/5/963/7056107
