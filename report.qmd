...
#Add in dentin wear of 0 at age 0.
dentin_long <-  bind_rows(dentin_long, dentin_long %>% filter(vital_status == "death") %>% mutate(age = 0, vital_status = "origin", DER = 0))
...
#ICC on model with eruption time point
...
mod_dentin_full <- lmer(DER ~ (age+I(age^2)+ I(age^3))*sex + (1|id) + (1|rater), data = dentin_long %>% filter(vital_status %in% c("death","life")) , REML = FALSE)
...
ggplot(aes(x = age, y = DER, color = sex), data = sub_dentin) + geom_point(alpha = 0.5) + geom_path(aes(x = age, y = isoy), data = sub_dentin, lwd = 2) + ggtitle("Dentin Ratio")
```
...
ggplot() + geom_point(aes(y = value, x = age, color = sex),data = suball, size = 0.5) + geom_path(aes(x = age, y = isoy, color = sex), data = suball, lwd = 2) + facet_grid(MD~BL) + theme_bw() + ylab("Scott Score") + xlab("Age (Months)") + scale_colour_manual(
...
ggplot() + geom_point(aes(y = value, x = age, color = sex),data = suball,size = 0.5) + geom_path(aes(x = age, y = isoy, color = sex), data = suball, lwd = 2) + facet_grid(MDword~BLword) + theme_bw() + ylab("Scott Score") + xlab("Age (Months)") + scale_colour_manual(
  values = c("#F8766D","#00BFC4"),
  labels = c( "Female","Male")
)
dev.off()
